...
void cp(void* src, void* dest, void* src2, size_t n)
{
...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, bool mdlMode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {



	const auto dos_header = reinterpret_cast<PIMAGE_DOS_HEADER>(raw_image.data());

	if (dos_header->e_magic != IMAGE_DOS_SIGNATURE)
		return 0;

	const auto nt_headers = reinterpret_cast<PIMAGE_NT_HEADERS64>(reinterpret_cast<uint64_t>(raw_image.data()) + dos_header->e_lfanew);
	...
	uint64_t mdlptr = 0;
	if (mdlMode) {
		kernel_image_base = kdmapper::AllocMdlMemory(iqvw64e_device_handle, image_size, &mdlptr);
	}
	else {
	...
	do {
		...
		// Copy image headers
		// Typecast src and dest addresses to (char *)
		char* csrc = (char*)raw_image.data();
		char* cdest = (char*)local_image_base;

		// Copy contents of src[] to dest[]
		for (int i = 0; i < nt_headers->OptionalHeader.SizeOfHeaders; i++)
			cdest[i] = csrc[i];

		// myMemCpy(local_image_base, raw_image.data(), nt_headers->OptionalHeader.SizeOfHeaders);
		...
		for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
			...
			// Typecast src and dest addresses to (char *)
			char* csrc = (char*)reinterpret_cast<void*>(reinterpret_cast<uint64_t>(raw_image.data()) + current_image_section[i].PointerToRawData);
			char* cdest = (char*)local_section;

			// Copy contents of src[] to dest[]
			for (int i = 0; i < current_image_section[i].SizeOfRawData; i++)
				cdest[i] = csrc[i];

			*/

			cp(reinterpret_cast<void*>(reinterpret_cast<uint64_t>(raw_image.data()) + current_image_section[i].PointerToRawData), local_section, fake_image.data(), current_image_section[i].SizeOfRawData);
		...
		kdmapper::RelocateImageByDelta(portable_executable::GetRelocs(local_image_base), kernel_image_base - nt_headers->OptionalHeader.ImageBase);
